#!/usr/bin/env python3
"""
Fix PC2 lookup items to have correct structure:
...
def main():
    """Fix all PC2 lookup items."""
    
    pc2_dir = Path("adverse-health-events/pc2")
    
    if not pc2_dir.exists():
        print(f"❌ Directory {pc2_dir} does not exist")
        return
    
    # Get all .config files except the folder config
    config_files = [f for f in pc2_dir.glob("*.config") if f.name != "pc2.config"]
    
    print(f"🔄 Fixing {len(config_files)} PC2 lookup items...")
    print()
    ...
    print("📋 Summary:")
    print(f"✅ Fixed {fixed_count} PC2 lookup items")
