#!/usr/bin/env python3
"""
Final fix for all PreValue Content Keys - replace with correct lookupItemFolder keys.
"""
...
# Mapping of PreValue file names to their correct lookupItemFolder Content Keys
CORRECT_KEYS = {
...
def fix_content_keys():
    """Fix all PreValue sources with correct Content Keys."""
    
    prevalue_dir = Path("prevalue-sources")
    updated = 0
    errors = 0
    
    print("🔧 Fixing PreValue Content Keys with correct lookupItemFolder references...")
    print()
    ...
    for prevalue_file in sorted(prevalue_dir.glob("*.config")):
        try:
            ...
            if folder_name not in CORRECT_KEYS:
                print(f"⚠️  No mapping for: {folder_name}")
                continue
            
            correct_key = CORRECT_KEYS[folder_name]
            ...
            # Check current key
            current_id_match = re.search(r'"id":"([^"]+)"', content)
            current_origin_match = re.search(r'"originKey":"([^"]+)"', content)
            
            if current_id_match:
                current_id = current_id_match.group(1)
                if current_id == correct_key:
                    print(f"✅ {folder_name}: Already correct ({correct_key})")
                    continue
                else:
                    print(f"🔄 {folder_name}: {current_id} → {correct_key}")
            
            # Replace both id and originKey
            content = re.sub(r'"id":"[^"]+"', f'"id":"{correct_key}"', content)
            content = re.sub(r'"originKey":"[^"]+"', f'"originKey":"{correct_key}"', content)
            ...
            print(f"❌ Error fixing {prevalue_file.name}: {e}")
            errors += 1
